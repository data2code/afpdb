...
from ..myalphafold.common import residue_constants
    ...
    if models is None or model in models:
      ...
      if line[:4] == "ATOM":
        ...
        if chains is None or chain in chains:
          ...
          # Let's support insertion
          #if resn[-1].isalpha(): # alternative atom
            # Let's support insertion
            #YZ: rather raise an exception
            #raise ValueError(
            #print("Warning:"
            #f'PDB contains an insertion code at chain {chain} and residue '
            #f'index {resn}. These are not supported.')
            ##resn = resn[:-1]
